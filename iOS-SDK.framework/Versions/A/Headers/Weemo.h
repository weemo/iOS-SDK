 ...
 * To accept the call, use -[call resume]. To deny it, use -[call hangup].  The WeemoCall object presents a  delegate of its own, decribed in the WeemoCall object.
 *
 ...
 * \param error If authentication failed, error will be different from nil. The debugDescription field of the NSError returns a NSString* describing the error in human terms
 */
 ...
 *
 * \param error nil if the Weemo singleton disconnected normally. The debugDescription field of the NSError returns a  NSString* describing the error in human terms.
 */
- (void)didDisconnect:(NSError*)error;

/**
 * Called after a contact is checked through the use of the -[Weemo canCallContact:]. While the contact availability is checked upon  -[[Weemo instance] call:] call, this delegate function is only called when the user/host application specifically call the -[[Weemo instance ]canCallContact:] method.
 *
 * This function is meant as a GUI updater, e.g. disabling a call button in case of !canBeCalled before displaying said button.
 *
 ...
 *
 * The -[[Weemo instance] foreground] and -[[Weemo instance] background] method are to be called everytime the host application goes to background/foreground. Failure to do so will result in undefined behavior.
 */
 ...
 * \param onInit This block will be called when Weemo has been properly initialized with (Weemo *, nil)  or when initialization has failed with (nil, NSError*) as arguments. The debugDescription field of the NSError returns a NSString* describing the error in human terms.
 */
 ...
 *
 * This will call the block immediately if Weemo has already been initialized or delay it's execution until Weemo is initialized. Those blocks are executed after the -[WeemoDelegate didConnect:] is called.
 *
...
/**
 * \brief Connects to servers with given userID. The connection is asynchronous, -[WeemoDelegate didConnect:] will be called upon instance initialization.
 *
...
/**
 * This function is to be called when the application comes to foreground. Not calling this function will result in undefined behavior.
 */
- (void)foreground;
